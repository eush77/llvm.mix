...
// CHECK-NEXT: define dso_local stage(2) float @paramstage(i32 stage(1) %a, float stage(2) %b) [[$attr:#[0-9]+]]
__stage(2) float paramstage(int __stage(1) a, float __stage(2) b) __stage(2) {
  return a + b;
}

// CHECK-LABEL: define dso_local i32* @fieldstage
...
// CHECK-LABEL: define dso_local %struct.node* @fieldstage1(%struct.node* %s)
...
// CHECK-LABEL: define dso_local %struct.node* @fieldstage2(%struct.node* %s)
...
// CHECK-LABEL: define dso_local i32 @fieldstage3(%struct.node1* %s)
