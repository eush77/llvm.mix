; RUN: opt -S -enable-new-pm=0 -mix %s -o - \
; RUN: | FileCheck %s --check-prefix=STAGE0 --implicit-check-not=define
; RUN: opt -S -enable-new-pm=0 -mix %s -o - | lli -force-interpreter - 2>&1 \
...
; STAGE0-DAG: [[param_types:%.+]] = alloca %struct.LLVMOpaqueType*
; STAGE0-DAG: [[param_ptr:%.+]] = getelementptr %struct.LLVMOpaqueType*, %struct.LLVMOpaqueType** [[param_types]], i32 0
; STAGE0-DAG: [[i32:%.+]] = call %struct.LLVMOpaqueType* @LLVMInt32TypeInContext
