; RUN: opt -S -enable-new-pm=0 -mix -globaldce %s \
; RUN: | FileCheck %s --check-prefix=STAGE0 --implicit-check-not=define
; RUN: opt -S -enable-new-pm=0 -mix -globaldce %s | lli -force-interpreter 2>&1 \
; RUN: | FileCheck %s --check-prefix=STAGE1
; RUN: opt -S -enable-new-pm=0 -mix -globaldce %s | lli -force-interpreter | opt -verify -disable-output
...
left:
  ; STAGE0: %p.left = call {{.*}} @left.mix
  %p.left = call i8* (i8*, ...) @llvm.mix.call(i8* bitcast (i32 (i32)* @left to i8*), i32 %x)
  br label %exit

right:
  %x1 = add i32 %x, 1
  ; STAGE0: %p.right = call {{.*}} @right.mix
  %p.right = call i8* (i8*, ...) @llvm.mix.call(i8* bitcast (i32 (i32)* @right to i8*), i32 %x1)
  br label %exit

; STAGE1-LABEL: {{^}}exit:
exit:
  ; STAGE1-NEXT: phi i8* [ bitcast (i32 ()* @left to i8*), %left ], [ bitcast (i32 ()* @right to i8*), %right ]
...
; STAGE0-LABEL: define {{.*}} @left
; STAGE0-LABEL: define {{.*}} @left.mix
; STAGE1: define internal i32 @left
; STAGE1-NEXT: ret i32 4
define i32 @left(i32 %x) stage(1) {
  ret i32 %x
}

; STAGE0-LABEL: define {{.*}} @right
; STAGE0-LABEL: define {{.*}} @right.mix
; STAGE1: define internal i32 @right
; STAGE1-NEXT: ret i32 5
define stage(1) i32 @right(i32 %x) stage(1) {
  ret i32 %x
}
