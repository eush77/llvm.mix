...
Params::Params(const Function &F, StagePredicate Pred,
  ...
  for (const Argument &A : F.args()) {
    ...
    Attrs.push_back(FA.getParamAttrs(A.getArgNo()));
  }
}
...
static void setMixFunctionAttributes(Function &F, const Function &SourceF) {
  ...
  F.addFnAttrs(AttrBuilder(SourceF.getAttributes().getAttributes(
                               AttributeList::FunctionIndex))
                   .removeAttribute(Attribute::Stage)
                   .addStageAttr(SourceF.getLastStage() - 1));
  F.addRetAttrs(AttrBuilder().addStageAttr(SourceF.getLastStage() - 1));
}
...
bool Mix::buildCall(CallInst &Call) const {
  ...
  Instruction *C = SB->stage(std::unique_ptr<Instruction, ValueDeleter>(
      CallInst::Create(CalleeInfo.FTy,
                       ConstantPointerNull::get(CalleeInfo.FTy->getPointerTo()),
